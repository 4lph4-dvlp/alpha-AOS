...
- [x] **SAFE-01**: User can preview any mutating alpha-AOS operation without changing source checkouts, global packages, configuration files, managed state, or harness resources
- [x] **SAFE-02**: User is protected from writes or removals that escape an allowed root through symlinks, junctions, reparse points, aliases, or changed parent paths
- [x] **SAFE-03**: User receives serialized, hash-checked, journaled, and crash-diagnosable managed writes so concurrent or interrupted operations cannot silently lose data
...
- [x] **SAFE-05**: User receives a clear refusal when catalogs, locks, manifests, evidence, receipts, or native configuration inputs are malformed, ambiguous, unsupported, or fail strict schema validation
...
- [x] **OPTO-01**: User can mark a directory as alpha-AOS `off`, have that policy apply to the directory and all descendants until an explicit nested override, and preview the effective launch policy before applying it
- [x] **OPTO-02**: User who configured an `off` tree once can later invoke each claimed harness through its ordinary supported command or application entrypoint without running an `alpha-aos run` command, and the isolation policy is enforced before that harness loads customizations
...
- [x] **REL-01**: User can run a single command that verifies the same packed release bytes install, reconcile, diagnose, and uninstall cleanly in Windows, macOS, and Linux fixture environments without touching real shared roots
- [x] **REL-02**: User can consult a versioned support matrix in which every active claimed harness and surface has at least one current real-host discovery and invocation canary, while Claude Code remains compatibility residue and other unproven combinations remain visibly limited or unsupported
