...
import { memoryHandoff, memorySearch, type MemoryCommandResult, type MemoryRunner } from "../../src/adapters/unified-memory.js";
import { comparePlanningTrees, hashPlanningTree, loadCanaryCatalog, matchExpectations } from "../../src/core/canary.js";
import { computeRiskSurfaceDigest, createGateReceipt } from "../../src/core/gate-receipt.js";
...
import { approveProjectPlan, planProjectCapabilities } from "../../src/core/project-plan.js";
import { applyUninstall, planUninstall } from "../../src/core/uninstall.js";
...
async function setupOfflineSources(supportRoot: string): Promise<{
  ...
  const packageRoot = join(supportRoot, "package-root");
  ...
  return { packageRoot, stateRoot, verifiedSourceRoot };
}
...
async function context7Proof(): Promise<BrownfieldLifecycleReport["context7"]> {
  ...
  const catalog = (await loadCanaryCatalog(repositoryRoot)).value;
...
export async function runBrownfieldLifecycle(fixture: BrownfieldFixture): Promise<BrownfieldLifecycleReport> {
  ...
  await timed("discuss", async () => {
    const observed = await hashPlanningTree(join(fixture.path, ".planning"));
    ...
    return { value: undefined, detail: `existing planning state ${observed.digest.slice(0, 12)} accepted` };
  });
  ...
  const executed = await timed("execute", async () => {
    ...
    return {
      ...
      detail: `Context7 ${context7.tools.join(" -> ")}; Codex -> Antigravity handoff retained`,
    };
  });
