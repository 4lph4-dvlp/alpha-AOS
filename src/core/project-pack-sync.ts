...
export async function planProjectPackSync(options: ProjectPackSyncOptions): Promise<ProjectPackSyncPlan> {
  ...
  for (const packId of packs) {
    ...
    const path = `${PROJECT_RECEIPT_DIRECTORY}/${packId}.json`;
    receipts.push({
      packId,
      path,
  ...
  return {
    ...
    proofs,
    digest,
    alreadyCurrent: await isAlreadyCurrent(targets, receipts),
  };
}
 ...
 *
 * Both halves matter. Skill bytes alone would report already-current for a pack
 * whose receipt was deleted, leaving provenance the reader cannot see; receipts
 * alone would report already-current for a pack whose bytes were replaced.
 */
...
export async function applyProjectPackSync(options: ApplyProjectPackSyncOptions): Promise<ProjectPackSyncResult> {
  ...
  return withComponentSession(reviewed, options.session, async (session): Promise<ProjectPackSyncResult> => {
    ...
    try {
      ...
      // The receipts join the SAME operations array. This is D-06: one
      // transaction, so a throw between the skill bytes and the receipt bytes
      // rolls back to zero receipts rather than leaving untracked writes.
