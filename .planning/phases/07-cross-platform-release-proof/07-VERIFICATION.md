...
gaps:
  - gap_id: G-07-2
    ...
    root_cause: "ATTEMPTED AND DISPROVEN 2026-09-22 (this supersedes the earlier reading that 'no further code change closes this gap — only running the real canary does'). The canary was actually run from an authenticated host. It spent a model turn and failed: the child session reached its init event, connected the fronted context7 server, then terminated at `result: \"Not logged in · Please run /login\"` with `terminal_reason: api_error`, `permission_denials: []` and exit 1. Cause: Claude Code stores its login inside CLAUDE_CONFIG_DIR (`~/.claude/.credentials.json`), and createIsolationLaunchSpec's claude branch replaces that directory to isolate configuration, skills, hooks, memory and MCP — hiding the credential along with them. No claude flag or variable names a credential source separately from the config root, and this stack never reads or copies authentication bytes (the same rule the codex branch states in its own warning). Codex escapes this only because its canary branch reuses the caller's native login boundary by reference; claude publishes no equivalent. A second defect compounded it: readiness reported `ready: true` with `blockedReasons: []` because its connection probe measures the USER's environment (it listed the user's own servers) rather than the isolated runtime the run would actually use, so the run was allowed to spend a turn that could only ever fail."
    artifacts:
    ...
    missing:
      - "A decision on HOW Claude is to be authenticated under canary isolation. Three candidates, none yet chosen: (a) supply ANTHROPIC_API_KEY to the launch, which the refusal already names and which authenticates by environment without touching the config root; (b) give claude a canary branch that reuses the caller's login boundary by reference, as the codex branch already does, which requires finding a claude-side equivalent that does not copy credential bytes; (c) accept that an isolated claude canary is not obtainable on this harness version and record it as a standing limitation."
      - "After a receipt exists by whichever route: reconcile .planning/REQUIREMENTS.md's REL-02 body text (still says Claude Code 'remains compatibility residue', stale under D-18 regardless of this gap)."
      - "Readiness honesty for claude: its connection probe measures the user's environment rather than the isolated runtime, so a `ready: true` answer does not describe the run that follows. Out of scope for this gap's closure but recorded here rather than left unstated."
    diagnosis_note: "The gap changed shape on evidence and is no longer a matter of simply spending a turn. The G-07-1 code defect stays closed and independently re-verified. What replaced it is a structural constraint of this harness version — Claude's credential and its configuration share one directory, so isolating the latter hides the former — plus a readiness probe that could not see that coming. The spend-before-refusal half is FIXED (the launch now records HARNESS_ISOLATION_UNPROVEN and does not launch; verified by an actual CLI run returning outcome blocked, launched false, exit 2). The authentication route itself is a human decision between the three candidates above, because two of them change a stated isolation guarantee."
deferred: []
---
