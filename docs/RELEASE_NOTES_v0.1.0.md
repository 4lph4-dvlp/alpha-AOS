...
A Claude canary runs on the login you already have. Claude Code keeps that login inside `CLAUDE_CONFIG_DIR`, and a canary runtime replaces that directory to isolate configuration, skills, hooks, memory, and MCP — so the runtime hard-links the existing login into itself. The bytes are never read or copied: the runtime holds a second directory entry for the one file, and removing the runtime removes only that entry. This is the same by-reference treatment the Codex canary already gives the caller's login. When no such link can be made — no login yet, or a managed state root on a different volume — the run is **refused before it spends a model turn** rather than spending one that could only end at `Not logged in`.

Canary runs also grant the harness the MCP servers the runtime fronts, by server and never by individual tool name. Without that grant an isolated run cannot reach its own observation front: a measured run recorded the harness choosing exactly the expected tool and being denied it, which alpha-AOS would otherwise have recorded as the harness failing to route.
